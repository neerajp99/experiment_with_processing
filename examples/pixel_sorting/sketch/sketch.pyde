...
def draw():
    ...
    for x in range(img.height):
        ...
        for i in range(len(newPixelArray) - 2):
            for j in range (len(newPixelArray) - 2):
                if (((newPixelArray[j][0] + newPixelArray[j][1] + newPixelArray[j][2]) * (mouseX/width + frameCount/1000)) < (newPixelArray[j+1][0] + newPixelArray[j+1][1] + newPixelArray[j+1][2])):
